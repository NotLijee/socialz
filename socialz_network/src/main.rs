...
async fn create_new_song(name: &str, artists: &str, genres: &str) -> Result<()> {
        ...
        if let Some(event) = evt {
            match event {
                ...
                EventType::Input(line) => match line.as_str() {
                    ...
                    cmd if cmd.starts_with("create songs") => handle_create_song(cmd).await,
                    cmd if cmd.starts_with("publish songs") => handle_publish_song(cmd).await,
...
async fn handle_list_songs(cmd: &str, swarm: &mut Swarm<SongBehaviour>) {
    let rest = cmd.strip_prefix("list song ");
    match rest {
        Some("all") => {
            let req = ListRequest {
                mode: ListMode::ALL,
...
async fn handle_create_song(cmd: &str) {
    if let Some(rest) = cmd.strip_prefix("create song ") {
        ...
        if elements.len() < 3 {
            info!("too few arguments - Format: name|artists|genres");
        } else {
            let name = elements.get(0).expect("name is there");
            let artists = elements.get(1).expect("artists is there");
            let genres = elements.get(2).expect("genres is there");
            if let Err(e) = create_new_song(name, artists, genres).await {
